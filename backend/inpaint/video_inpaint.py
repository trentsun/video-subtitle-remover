    ...
    def inpaint(self, frames, mask):
        try:
            ...
            if w % 8 != 0 or h % 8 != 0:
                new_w = ((w + 7) // 8) * 8
                new_h = ((h + 7) // 8) * 8
                print(f"调整帧尺寸从 {(w, h)} 到 {(new_w, new_h)}")
                frames = [f.resize((new_w, new_h), Image.BILINEAR) for f in frames]
            ...
            # 验证掩码
            if not flow_masks or not masks_dilated:
                raise ValueError("掩码生成失败")
            
            # 打印调试信息
            print(f"转换后的尺寸信息:")
            print(f"帧尺寸: {frames[0].size}")
            print(f"掩码尺寸: {mask.shape if isinstance(mask, np.ndarray) else 'unknown'}")
            ...
            print(f"张量尺寸:")
            print(f"frames_tensor: {frames_tensor.shape}")
            print(f"flow_masks_tensor: {flow_masks_tensor.shape}")
            print(f"masks_dilated_tensor: {masks_dilated_tensor.shape}")
            
            if frames_tensor.numel() == 0 or flow_masks_tensor.numel() == 0 or masks_dilated_tensor.numel() == 0:
                raise ValueError("张量转换后大小为0")
            ...
            with torch.no_grad():
                ...
                if frames_tensor.size(1) > short_clip_len:
                    ...
                    for f in range(0, video_length, short_clip_len):
                        ...
                        if f == 0:
                            flows_f, flows_b = self.fix_raft(frames_tensor[:, f:end_f], iters=self.raft_iter)
                        else:
                            flows_f, flows_b = self.fix_raft(frames_tensor[:, f - 1:end_f], iters=self.raft_iter)
                    ...
                    gt_flows_bi = self.fix_raft(frames_tensor, iters=self.raft_iter)
                    torch.cuda.empty_cache()

                if self.use_half:
                    frames_tensor, flow_masks_tensor, masks_dilated_tensor = frames_tensor.half(), flow_masks_tensor.half(), masks_dilated_tensor.half()
                ...
                if video_length > subvideo_length_img_prop:
                    ...
                    prop_imgs, updated_local_masks = self.model.img_propagation(masked_frames, pred_flows_bi, masks_dilated_tensor,
                                                                           'nearest')
            ...
            for f in range(0, video_length, neighbor_stride):
                ...
                with torch.no_grad():
                    ...
                    binary_masks = masks_dilated_tensor[0, neighbor_ids, :, :, :].cpu().permute(
